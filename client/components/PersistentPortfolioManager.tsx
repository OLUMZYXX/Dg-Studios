...
export default function PersistentPortfolioManager({
  ...
  const toggleShowcase = (id: string) => {
    let updatedIds = showcaseIds.includes(id)
      ? showcaseIds.filter((sid) => sid !== id)
      : [...showcaseIds, id]
    if (updatedIds.length > 7) {
      setModal({
        open: true,
        message: 'You can only showcase up to 7 images on the home page.',
                  ...
                  {filteredItems.map((item, index) => (
                    ...
                    >
                      {(provided, snapshot) => (
                        ...
                        >
                          {/* Drag Handle (Admin Only) - More Visible */}
                          {isAdmin && (
                            <div
                              ...
                              <div className='absolute bottom-3 right-3 z-30'>
                                <label className='flex items-center space-x-2 bg-white/80 px-3 py-1 rounded-lg shadow border cursor-pointer'>
                                  <input
                                    ...
                                    disabled={
                                      !showcaseIds.includes(item.id) &&
                                      showcaseIds.length >= 7
                                    }
                                  />
                          ...
                          {/* Quick Delete Button (Top Right Corner) - More Visible */}
